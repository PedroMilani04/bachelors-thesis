...
FIXED_FEATURES = [
    ...
    'Selic_Delta_63d',
    'Spread_BR_US',
    'Spread_BR_US_Delta_21d',
    'MACD_Signal',
    'WTI_Retorno_63d_lag3'
]
...
def xgboostModel():
    ...
    print("\n" + "="*70)
    print("   XGBOOST COM FEATURES FIXAS (PETROLEUM)")
    print("   Colunas: DiasParaEleicaoBR, DiasParaEleicaoUSA, Selic_Delta_21d,")
    print("            Fed_Delta_63d, Selic_Delta_63d, Spread_BR_US,")
    print("            Spread_BR_US_Delta_21d, MACD_Signal, WTI_Retorno_63d_lag3")
    print("="*70 + "\n")
    ...
    print(f"   Shape TREINO (antes de selecionar features): {X_train.shape}")
    print(f"   Shape TESTE (antes de selecionar features):  {X_test.shape}")
    print(f"   Distribuição: {np.bincount(y_train.astype(int))} (Venda / Neutro / Compra)")

    # --- SELEÇÃO DE FEATURES FIXAS ---
    print("\n--- 2.5. SELEÇÃO DE FEATURES FIXAS ---")
    print(f"   Features disponíveis: {list(X_train.columns)}")
    
    # Verifica quais colunas existem
    missing_features = [f for f in FIXED_FEATURES if f not in X_train.columns]
    available_features = [f for f in FIXED_FEATURES if f in X_train.columns]
    ...
    print(f"   Shape TREINO (após seleção): {X_train.shape}")
    print(f"   Shape TESTE (após seleção):  {X_test.shape}")
    ...
    print("\n" + "="*60)
    print(f"   ACURÁCIA (3 CLASSES - FEATURES FIXAS): {acc:.2%}")
    print("="*60 + "\n")
    ...
    texto_relatorio = (
        f"--- RESULTADO (FEATURES FIXAS) ---\n"
        f"Features: {len(available_features)}/9\n\n"
    ...
    plt.tight_layout()
    # Salva na RAIZ
    output_path_img = os.path.join(current_dir, 'matriz-fixed-features-3-classes.png')
    ...
    # Salva na RAIZ
    export_path = os.path.join(current_dir, 'features_treino_teste_fixed_3_classes.csv')
